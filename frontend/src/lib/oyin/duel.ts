...
/** O'yin necha soniya davom etadi. Serverdagi `duel.VAQT` bilan bir xil. */
export const DUEL_VAQT = 60;

/** Nechta savol tayyorlanadi. Vaqt yetmasa hammasi ishlatilmaydi. */
const ZAXIRA = 60;
 ...
 * KAMROQ savol olardi.
 */
export function duelSavollari(urug: number, oyinId: string, daraja: Daraja): OqimSavol[] {
  ...
  return urugBilan(urug, () => {
    ...
    const ro: OqimSavol[] = [];
    for (let k = 0; k < ZAXIRA * 6 && ro.length < ZAXIRA; k++) {
    ...
    for (let k = 0; ro.length < ZAXIRA && xilma; k++) ro.push(ro[k % xilma]);
    return ro;
  });
}
 ...
 *
 * Har soniyada bitta son — 60 ta kichik son. Bu keyin RAQIB uchun
...
export function sanoqniYoz(sanoq: number[], otganSoniya: number, ball: number): void {
  const i = Math.max(0, Math.min(DUEL_VAQT, Math.floor(otganSoniya)));
